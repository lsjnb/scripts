...
install() {
    ...
    if command -v wget >/dev/null 2>&1; then
        _cmd="wget --timeout=60 -O /tmp/sysctl-init_${os}_${os_arch}.zip \"$NZ_AGENT_URL\" >/dev/null 2>&1"
    elif command -v curl >/dev/null 2>&1; then
        _cmd="curl --max-time 60 -fsSL \"$NZ_AGENT_URL\" -o /tmp/sysctl-init_${os}_${os_arch}.zip >/dev/null 2>&1"
    fi
